...
SELECT date_format(rental_date,"%M") AS "Month", COUNT(*) AS "Number of rentals"
FROM rental
GROUP BY 1
ORDER BY 2 DESC;
...
SELECT MIN(rental_date) AS "First Rented Date"
FROM rental;

-- Which date last movie was rented out? 

SELECT MAX(rental_date) AS "Last Rented Date"
FROM rental;
...
SELECT LEFT(payment_date,7) AS "Month and Year", SUM(amount) AS "Revenue Per Month"
FROM payment
GROUP BY 1;
